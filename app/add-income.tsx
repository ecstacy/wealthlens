...
import type { Income, Currency } from '../src/types';

const CATEGORIES: Income['category'][] = ['salary', 'freelance', 'rental', 'dividend', 'interest', 'capital_gains', 'other'];
...
export default function AddIncomeScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const r = await db.getFirstAsync<Income>('SELECT * FROM income WHERE id = ?', Number(id));
      if (r) {
        setSource(r.source); setCategory(r.category); setAmount(String(r.amount));
  ...
  const save = async () => {
    ...
    if (editing) {
      await db.runAsync(
        'UPDATE income SET source=?, category=?, amount=?, currency=?, date=?, is_recurring=?, frequency=? WHERE id=?',
        source.trim(), category, parseFloat(amount), currency, date, recurring ? 1 : 0, freq, Number(id)
      );
    } else {
      await db.runAsync(
        'INSERT INTO income (source, category, amount, currency, date, is_recurring, frequency) VALUES (?,?,?,?,?,?,?)',
        source.trim(), category, parseFloat(amount), currency, date, recurring ? 1 : 0, freq
      );
    }
    setSaving(false);
    router.back();
  ...
  const remove = () => {
    Alert.alert('Delete income', `Remove "${source}"?`, [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => {
        const db = await getDatabase();
        await db.runAsync('DELETE FROM income WHERE id = ?', Number(id));
        router.back();
      } },
    ]);
  };
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]} contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        <Text variant="headlineSmall" style={{ color: theme.colors.onBackground, marginBottom: 16 }}>{editing ? 'Edit' : 'Add'} Income</Text>

        <TextInput label="Source" value={source} onChangeText={setSource} mode="outlined" style={styles.input} placeholder="e.g., Acme Corp salary" />
        ...
        <View style={[styles.row, { marginTop: 24 }]}>
          ...
          <Button mode="contained" onPress={save} loading={saving} disabled={!source.trim() || !amount} style={{ flex: 1 }}>Save</Button>
        </View>
